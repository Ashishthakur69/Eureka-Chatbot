    ...
    [
        (
            ...
            (
                ...
                "- Sports scores\n"
                "- Stock prices\n\n"
                "If the question is general knowledge, answer directly without searching.\n"
                "Always synthesize results and be honest if you cannot find an answer."
            ),
        ),
...
def upload_file():
    ...
    if embeddings is None:
        return jsonify({"error": "Embeddings model is not available."}), 500
    ...
    if file:
        ...
        try:
            ...
            session_state.rag_chain = RetrievalQA.from_chain_type(
...
def clear_document():
    ...
    session_state.history.clear()
    return jsonify({"success": True, "message": "Document context cleared."}), 200
    ...
    def generate_response():
        try:
            if session_state.rag_chain:
                ...
                yield response_data.get("result", "No answer found in the document.")
            else:
...
def serve_frontend():
    ...
    return "🚀 Hugging Face Space is running!", 200
